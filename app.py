...
class threadClass:
    ...
    def run(self):
        ...
        collection_name = self.scrapper_object.getReviewsToDisplay(expected_review=self.expected_review,
                                                                   searchString=self.searchString, username='mugunth',
                                                                   password='perumal',
...
def index():
    if request.method == 'POST':
        ...
        try:
            review_count = 0
            scrapper_object = FlipkratScrapper(executable_path=ChromeDriverManager().driver_path,
                                               chrome_options=chrome_options)
            mongoClient = MongoDBManagement(username='mugunth', password='perumal')
